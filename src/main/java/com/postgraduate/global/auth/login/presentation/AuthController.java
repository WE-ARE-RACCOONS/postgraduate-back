...
public class AuthController {
    ...
    public ResponseEntity<ResponseDto<AuthResponse>> authLoginWithToken(@RequestBody @Valid TokenRequest request, @PathVariable Provider provider, HttpServletResponse response) {
        ...
        JwtTokenResponse jwtToken = jwtUseCase.signIn(authUser.user(), response);
    ...
    @Operation(summary = "소셜 로그인", description = "회원인 경우 JWT를, 회원이 아닌 경우 socialId를 반환합니다(회원가입은 진행하지 않습니다). 탈퇴 회원인 경우 isDelete = true 가 응답됩니다")
    public ResponseEntity<ResponseDto<AuthResponse>> authLogin(@RequestBody @Valid CodeRequest request, @PathVariable Provider provider, HttpServletResponse response) {
        SignInUseCase signInUseCase = selectOauth.selectSignIn(provider);
    ...
    public ResponseEntity<ResponseDto<AuthResponse>> authDevLogin(@RequestBody @Valid CodeRequest request, @PathVariable Provider provider, HttpServletResponse response) {
        SignInUseCase signInUseCase = selectOauth.selectSignIn(provider);
    ...
    public ResponseEntity<ResponseDto<AuthResponse>> reJoin(@PathVariable Provider provider, @RequestBody RejoinRequest request, HttpServletResponse response) {
        User user = userManageUseCase.updateRejoin(provider, request);
        JwtTokenResponse jwtToken = jwtUseCase.signIn(user, response);
        return ResponseEntity.ok(create(AuthResponseCode.AUTH_ALREADY.getCode(), AuthResponseMessage.SUCCESS_AUTH.getMessage(), jwtToken));
    }

    @PostMapping("/logout")
    @Operation(summary = "로그아웃", description = "토큰 같이 보내주세요")
    public ResponseEntity<ResponseDto<Void>> logout(@AuthenticationPrincipal User user, HttpServletResponse response) {
        jwtUseCase.logout(user, response);
    ...
    public ResponseEntity<ResponseDto<JwtTokenResponse>> signUpUser(@RequestBody @Valid SignUpRequest request, HttpServletResponse response) {
    ...
    public ResponseEntity<ResponseDto<JwtTokenResponse>> changeUserToken(@AuthenticationPrincipal User user, HttpServletResponse response) {
        signUpUseCase.changeUser(user);
        JwtTokenResponse jwtToken = jwtUseCase.changeUser(user, response);
        return ResponseEntity.ok(create(AuthResponseCode.AUTH_CREATE.getCode(), AuthResponseMessage.SUCCESS_AUTH.getMessage(), jwtToken));
    }

    @PostMapping("/senior/signup")
    @Operation(summary = "대학원생 가입 - 필수 과정만", description = "대학원생 회원가입 - 필수 과정만")
    public ResponseEntity<ResponseDto<JwtTokenResponse>> singUpSenior(@RequestBody @Valid SeniorSignUpRequest request, HttpServletResponse response) {
    ...
    public ResponseEntity<ResponseDto<JwtTokenResponse>> changeSenior(@AuthenticationPrincipal User user,
        ...
        JwtTokenResponse jwtToken = jwtUseCase.changeSenior(changeUser, response);
        return ResponseEntity.ok(create(SENIOR_CREATE.getCode(), CREATE_SENIOR.getMessage(), jwtToken));
    }

    @PostMapping("/senior/token")
    @Operation(summary = "선배로 변경 | 토큰 필요", description = "선배로 변경 가능한 경우 선배 토큰 발급")
    public ResponseEntity<ResponseDto<JwtTokenResponse>> changeSeniorToken(@AuthenticationPrincipal User user, HttpServletResponse response) {
        JwtTokenResponse jwtToken = jwtUseCase.changeSenior(user, response);
        return ResponseEntity.ok(create(AuthResponseCode.AUTH_CREATE.getCode(), AuthResponseMessage.SUCCESS_AUTH.getMessage(), jwtToken));
    }

    @PostMapping("/refresh")
    @Operation(summary = "토큰 재발급 | 토큰 필요", description = "refreshToken 으로 토큰 재발급")
    public ResponseEntity<ResponseDto<JwtTokenResponse>> refresh(@AuthenticationPrincipal User user, HttpServletRequest request, HttpServletResponse response) {
        JwtTokenResponse jwtToken = jwtUseCase.regenerateToken(user, request, response);
